...
const Alert = (props: ThemedAlertProps) => {
	...
	if (plaintext) {
		return (
			<Flex color={color} {...restProps}>
				{icon && (
					<Txt fontSize={2} mb="auto" mr={2}>
						<FontAwesomeIcon icon={icon} />
					</Txt>
				)}
				<Txt.span fontSize={2}>{props.children}</Txt.span>
			</Flex>
		);
	}
	...
	return (
		...
		>
			{icon && (
				<Txt fontSize={2} mb="auto" mr={3}>
					<FontAwesomeIcon icon={icon} />
				</Txt>
			)}
			<Box flex={1}>
				{title && (
					<Txt.span fontSize={2} bold mr={2}>
						{title}
					</Txt.span>
				)}
				<Txt.span fontSize={2} color={emphasized ? 'white' : 'text.main'}>
					{props.children}
				</Txt.span>
			</Box>
		</DismissableContainer>
